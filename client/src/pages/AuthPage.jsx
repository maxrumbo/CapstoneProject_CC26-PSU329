...
function ForgotForm({ onSwitch, toast }) {
  ...
  const submitEmail = async (e) => {
    e.preventDefault();
    if (!isValidEmail(email)) return setErrors((prev) => ({ ...prev, email: "Format email tidak valid." }));
    setErrors((prev) => ({ ...prev, email: "" }));
    setLoading(true); await new Promise(r => setTimeout(r, 1000)); setLoading(false);
    toast(`OTP sent to ${email}`, "info"); setStep(1);
  };
  ...
  const submitNewPw = async (e) => {
    ...
    setLoading(true); await new Promise(r => setTimeout(r, 1000)); setLoading(false);
    onSwitch("login"); setTimeout(() => toast("Password updated! Please sign in.", "success"), 100);
  };
